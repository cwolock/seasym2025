...
* Writing your own regression estimator that is compatible with `SuperLearner`.
...
# Installing \texttt{R} packages

To follow along with this demonstration, you will need several \texttt{R} packages. \vspace{-0.15in} 

\begin{itemize} \setlength\itemsep{0em}
	\item Packages are freely available from the Comprehensive R Archive Network (CRAN).
	\item Packages are downloaded to your local computer via the \texttt{install.packages} function.
	\item Packages are loaded into your current \texttt{R} session via \texttt{require} or \texttt{library}
\end{itemize}

```{r installpack, message=FALSE, eval = FALSE}
...
\begin{itemize} \setlength\itemsep{0em}
	...
	\item Conditional distribution of $Y$ given $(A,W) = (a,w)$ is Normal with mean $\bar{Q}(a,w) = w_1 - w_2 + a$.
\end{itemize}
...
For later use, we will define a function that generates simulated data. 

```{r sim_data_fn}
# the function takes sample size, n, as input
make_data <- function(n){
	...
	all_data <- data.frame(W1 = W1, W2 = W2, A = A, Y = Y)
...
Our first exercise involves comparing two estimates of $\bar{Q}(a,w) = E[Y \midd A=a, W=w]$. \vspace{-0.15in} 
...
\textcolor{blue}{The two-fold cross-validated risk of $\bar{Q}_2$ is computed below}

```{r two_fold_cv2_answer}
# fit Q2 in split 1
...
# Q1_fit and Q2_fit are fitted glms
...
* Theory says we do well eventually (in the sample size sense), but did we do well in these data?
...
* Include flexible learners -- `SL.earth` (polynomial multivariate adaptive regression splines), `SL.xgboost` (gradient boosted machines), and `SL.ranger` (fast random forests) are good examples. 
